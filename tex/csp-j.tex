...
用二分搜索找到元素7并输出元素下标，1, 3, 5, 7, 9, 11, 13, 15\\
解:\\
\begin{tabular}{|c|c|c|}
\hline
...
\section{factorial}
从factorial.in文件中读取数字num，计算num的阶乘，将结果写入文件factorial.out\\
解:\\
\begin{lstlisting}[language=c++,breaklines=true]
#include <iostream>
#include <fstream>
...
2^2 + 2^1 + 2^0$ 就不是一个优秀的拆分，因为 1 不是 2 的正整数次幂。\\
...
1 & $N$头牛抵达机场 & $1\leq N\leq 10^5$\\
\hline
2 & 第$i$头牛在时间$t_i$抵达 & $0\leq t_i\leq 10^9$\\
\hline
3 & $M$辆巴士来运牛从机场到农场 & $1\leq M\leq 10^5$\\
\hline
4 & 每辆巴士最多可以容纳$C$头牛 & $1\leq C\leq N$\\
\hline
4 & input & 6 3 2\\
\hline
4 & output & 4\\
...
的，这种表达式有且仅有以下几种运算：\\
1. 与运算： $�a\&�b$�。当且仅当�a和�b的值都为1时，该表达式的值为1。其余情况该表达式的值为0。\\
